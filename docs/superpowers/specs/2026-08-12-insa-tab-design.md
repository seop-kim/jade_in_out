...
기존 Jade 출퇴근 기록 화면은 그대로 유지하면서, 상단 탭으로 분리된 `신규 인사시스템` 화면에서 `https://insa.kwe.co.kr`의 HTML 응답을 Cookie 인증으로 가져온다. 신규 화면은 개인 출퇴근 기록, 내 휴가 사용내역, 다른 직원의 날짜별 휴가·시간 일정 정보를 하나의 신규 달력에 표시한다.
...
- 휴가 페이지의 개인 사용내역은 달력 날짜에 표시하고, 연간 발생·사용·잔여시간 요약은 표시하지 않는다.
...
5. 내 휴가 사용내역은 날짜별 달력 영역에 표시하고 연간 휴가 현황 요약 영역은 두지 않는다.
...
## 후속 보완: 날짜 상세 hover 툴팁

신규 INSA 달력에서 팀 일정 건수가 있는 날짜에 마우스를 올리면 기존 Jade 달력과 같은 어두운 고정 위치 툴팁을 표시한다. 키보드 사용자는 날짜 버튼에 focus해 같은 정보를 볼 수 있다.

- hover/focus가 시작되면 해당 날짜의 `/main.asp` 상세를 지연 조회한다.
- 툴팁은 조회 중, 조회 실패, 빈 일정, 상세 목록 상태를 모두 표시한다.
- 상세 목록은 이름, 일정명, 기간을 보여 주며 Jade 툴팁과 동일한 폭·색상·날짜 헤더·화면 경계 보정 규칙을 사용한다.
- mouse leave/blur에서는 툴팁만 닫고 성공한 상세 결과는 기존 날짜별 캐시에 남긴다.
- 날짜 클릭은 기존 하단 상세 패널을 계속 열며 hover가 받아 둔 캐시를 재사용한다.
- hover만으로 하단 상세 패널을 열거나 선택 날짜를 변경하지 않는다.
- 팀 일정 건수가 없는 날짜는 상세 요청과 툴팁을 만들지 않는다.
